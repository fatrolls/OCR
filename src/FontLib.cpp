...
void OCR_UnloadFonts()
{
	for (auto itr = CachedFonts.begin(); itr != CachedFonts.end(); itr++)
	{
		FontImg* i = *itr;
		free(i->AssignedString);
		free(i->Pixels);
		if (i->PixelsBlurred)
			free(i->PixelsBlurred);
		free(i);
	}
	CachedFonts.clear();
}
...
FontSimilarityScore* GetBestMatchedFont_ShapeUpScale(FontExtracted* f)
{
	...
	for (auto itr = CachedFonts.begin(); itr != CachedFonts.end(); itr++)
	{
		...
		if (FontRatio * 0.8 > ShapeRatio || FontRatio * 1.2 < ShapeRatio)
			continue;
#endif
#if defined( ONLY_UPSCALE_SHAPES) || 1
		if ( ABS(f->Width - i->Width) > 2 || ABS(f->Height - i->Height) > 2 )
			continue;
#endif
		//scale the shape to the size of the current font image
...
#endif
#if defined( USE_HAIRLINE_FONTS) || 0 
		ErodeToLines(ScaledShape, i->Width, i->Height, pitchRescaled, 128);
		ErodeToLines(ScaledShape, i->Width, i->Height, pitchRescaled, 128);
		BlurrImageToGrayScale(ScaledShape, i->Width, i->Height, pitchRescaled, 3);
#endif
#if defined( _DEBUG ) && 1
		SaveImagePNG(ScaledShape, f->Width, f->Height, pitchShape, "compareOldToThisFont.png");
#endif
//		BlurrImageToGrayScale(ScaledShape, i->Width, i->Height, pitchRescaled, 3);
...
//				__int64 SAD = Img_SAD_FontPresent(i->Pixels, MinWidth, MinHeight, i->pitch, Shape, tx, ty, pitchShape);
				...
				if (SAD < SADNow)
				{
					BestY = ty;
					BestX = tx;
					SADNow = (__int64)sqrt((double)(10000*SAD));
				}
			}
...
FontSimilarityScore* GetBestMatchedFont_DirectSAD(FontExtracted* f)
{
	FontSimilarityScore* ret = (FontSimilarityScore*)malloc(sizeof(FontSimilarityScore));
	memset(ret, 0, sizeof(FontSimilarityScore));
	ret->SAD = 0x07FFFFFFF;
	...
	{
		FontImg* i = (*itr);
		if (ABS(f->Width - i->Width) > sSettings.FontSizeDeviation || ABS(f->Height - i->Height) > sSettings.FontSizeDeviation)
			continue;
#if defined( _DEBUG ) && 1
		SaveImagePNG(i->Pixels, i->Width, i->Height, i->pitch, "compareOldToThisFont.png");
#endif
		//get the SAD of the 2 images
		int PixelCountDiff;
		__int64 SADNow = Img_SAD_CountPixels(i->Pixels, i->Width, i->Height, i->pitch, f->Pixels, pitchShape, &PixelCountDiff);

		if (sSettings.ExactMatchInsteadBestMatch == 1 && SADNow != 0)
			continue;
		if (sSettings.MaxAllowedPixelDifferences < PixelCountDiff)
			continue;
...
FontSimilarityScore* GetBestMatchedFontAtAreat_DirectSAD(FIBITMAP* dib, FontExtracted* f, int SearchAreaSize)
{
	FontSimilarityScore* ret = (FontSimilarityScore*)malloc(sizeof(FontSimilarityScore));
	memset(ret, 0, sizeof(FontSimilarityScore));
#if defined( _DEBUG ) && 0
	...
	int x = f->x;
	int y = f->y;
	ret->SAD = 0x00FFFFFFFFFFFFFF;
	int Width = FreeImage_GetWidth(dib);
	int Height = FreeImage_GetHeight(dib);
	BYTE* Pixels = FreeImage_GetBits(dib);
	int pitch = FreeImage_GetPitch(dib);
	for (int y1 = y - SearchAreaSize; y1 <= y + SearchAreaSize; y1++)
		for (int x1 = x - SearchAreaSize; x1 <= x + SearchAreaSize; x1++)
		{
			...
			for (auto itr = CachedFonts.begin(); itr != CachedFonts.end(); itr++)
			{
				...
				{
					SaveImagePNG(i->Pixels, i->Width, i->Height, i->pitch, "compareOldToThisFont.png");
					SaveImagePNG(Pixels + y1 * pitch + x1 * Bytespp, i->Width, i->Height, pitch, "CompareToThisImage.png");
				}
#endif
				__int64 SADNow = Img_SAD_CountPixels(i->Pixels, i->Width, i->Height, i->pitch, Pixels + y1 * pitch + x1 * Bytespp, pitch, &PixelCountDiff);
...
//	return GetBestMatchedFont_ShapeUpScale(f);
	...
	if(sSettings.AreaScan == 0)
		return GetBestMatchedFont_DirectSAD(f);
	else
		return GetBestMatchedFontAtAreat_DirectSAD(dib, f, sSettings.AreaScan);
}
...
void OCR_ThinFontLines()
{
	for (auto itr = CachedFonts.begin(); itr != CachedFonts.end(); itr++)
	{
		if ((*itr)->LoadedFromFile != 1)
			continue;
		ErodeToLines((*itr)->Pixels, (*itr)->Width, (*itr)->Height, (*itr)->pitch, 128);
	}
}