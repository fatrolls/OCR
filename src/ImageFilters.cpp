...
__int64 Img_SAD_CountPixels(BYTE* Pixels1, int Width1, int Height1, int pitch1, BYTE* Pixels2, int pitch2, int* out_PixelCount)
{
	*out_PixelCount = 0;
	__int64 ret = 0;
	for (int y = 0; y < Height1; y++)
...
__int64 Img_SAD_CountPixels(BYTE* Pixels1, int Width1, int Height1, int pitch1, BYTE* Pixels2, int atx, int aty, int pitch2, int* out_PixelCount)
{
	__int64 ret = 0;
	*out_PixelCount = 0;
	for (int y = 0; y < Height1; y++)
...
void ErodeOutside(BYTE* Pixels, int Width, int Height, int pitch, int ErrodeIfSmaller)
{
	...
	for (int y = 1; y < Height - 1; y++)
		for (int x = 1; x < Width - 1; x++)
		{
			...
			BYTE Kernel1[3][3] = { {0,0,0},{0,0,0},{255,255,255} };
			BYTE Kernel2[3][3] = { {0,0,255},{0,0,255},{0,0,255} };
			BYTE Kernel3[3][3] = { {255,255,255},{0,0,0},{0,0,0} };
			BYTE Kernel4[3][3] = { {255,0,0},{255,0,0},{255,0,0} };
			BYTE Kernel5[3][3] = { {255,0,0},{255,0,0},{255,255,0} };
			BYTE Kernel6[3][3] = { {0,0,0},{0,0,255},{255,255,255} };
			BYTE Kernel7[3][3] = { {0,255,255},{0,0,255},{0,0,255} };
			BYTE Kernel8[3][3] = { {255,255,255},{255,0,0},{0,0,0} };
			BYTE Kernel9[3][3] = { {255,255,0},{255,0,0},{0,0,0} };
			BYTE Kernel10[3][3] = { {0,0,0},{255,0,0},{255,255,0} };
			BYTE Kernel11[3][3] = { {0,0,0},{0,0,255},{0,255,255} };
			BYTE Kernel12[3][3] = { {0,255,255},{0,0,255},{0,0,0} };
			int ty = y - 1;
			int tx = x - 1;
...
int ErodeToLines(BYTE* Pixels, int Width, int Height, int pitch, int ErrodeIfSmaller)
{
	int* VertSizes = (int*)malloc(Height * Width * sizeof(int));
	int* HorSizes = (int*)malloc(Height * Width * sizeof(int));
	memset(VertSizes, 0, Height * Width * sizeof(int));
	memset(HorSizes, 0, Height * Width * sizeof(int));
	BYTE* TPixels = (BYTE*)malloc(Height * pitch);
	memcpy(TPixels, Pixels, Height * pitch);
	//get vertical sizes for each pixel
	for(int y=0;y<Height;y++)
		for (int x = 0; x < Width; x++)
		{
			//errode black pixels, skip rest
			if (Pixels[y * pitch + x * Bytespp] > ErrodeIfSmaller)
				continue;
			if (VertSizes[y * Width + x] == 0)
			{
				int len = 1;
				for (int y2 = y + 1; y2 < Height; y2++)
					if (Pixels[y2 * pitch + x * Bytespp] > ErrodeIfSmaller)
						break;
					else
						len++;
				for (int y2 = y; y2 < y + len; y2++)
					VertSizes[y2 * Width + x] = len;
			}
			if (HorSizes[y * Width + x] == 0)
			{
				int len = 1;
				for (int x2 = x + 1; x2 < Width; x2++)
					if (Pixels[y * pitch + x2 * Bytespp] > ErrodeIfSmaller)
						break;
					else
						len++;
				for (int x2 = x; x2 < x + len; x2++)
					HorSizes[y * Width + x2] = len;
			}
		}
	//do the actual erode
	int ErrodeMade = 1;
//	while (ErrodeMade > 0)
	{
		...
		for (int y = 0; y < Height; y++)
			for (int x = 0; x < Width; x++)
			{
				//non erodable pixels are skipped
				if (Pixels[y * pitch + x * Bytespp] > ErrodeIfSmaller)
					continue;
				//if this is a horizontal line ( because it's thicker)
				//but make sure we do not eat single lines
				if (VertSizes[y * Width + x] < HorSizes[y * Width + x])
				{
					//upper pixel is white, lower pixel is black
					if ((y - 1 < 0 || Pixels[(y - 1) * pitch + x * Bytespp] > ErrodeIfSmaller)
						&& (y + 1 < Height && Pixels[(y + 1) * pitch + x * Bytespp] <= ErrodeIfSmaller)
						&& (y + 1 < Height && TPixels[(y + 1) * pitch + x * Bytespp] <= ErrodeIfSmaller)
						)
					{
						TPixels[y * pitch + x * Bytespp + 0] = 255;
						TPixels[y * pitch + x * Bytespp + 1] = 255;
						TPixels[y * pitch + x * Bytespp + 2] = 255;
						ErrodeMade++;
					}
					//lower pixel is white, upper pixel is black
					if ((y + 1 >= Height || Pixels[(y + 1) * pitch + x * Bytespp] > ErrodeIfSmaller)
						&& (y - 1 >= 0 && Pixels[(y - 1) * pitch + x * Bytespp] <= ErrodeIfSmaller)
						&& (y - 1 >= 0 && TPixels[(y - 1) * pitch + x * Bytespp] <= ErrodeIfSmaller)
						)
					{
						TPixels[y * pitch + x * Bytespp + 0] = 255;
						TPixels[y * pitch + x * Bytespp + 1] = 255;
					...
					//if(VertSizes[y * Width + x] > HorSizes[y * Width + x])
				{
					//left pixel is white, right pixel is black
					if ((x - 1 < 0 || Pixels[y * pitch + (x - 1) * Bytespp] > ErrodeIfSmaller)
						&& (x + 1 < Width && Pixels[y * pitch + (x + 1) * Bytespp] <= ErrodeIfSmaller)
						&& (x + 1 < Width && TPixels[y * pitch + (x + 1) * Bytespp] <= ErrodeIfSmaller)
						)
					{
						TPixels[y * pitch + x * Bytespp + 0] = 255;
						TPixels[y * pitch + x * Bytespp + 1] = 255;
					...
					//to the right there is a white pixel, but to the left there is black pixel
					if ((x + 1 >= Width || Pixels[y * pitch + (x + 1) * Bytespp] > ErrodeIfSmaller)
						&& (x - 1 >= 0 && Pixels[y * pitch + (x - 1) * Bytespp] <= ErrodeIfSmaller)
						&& (x - 1 >= 0 && TPixels[y * pitch + (x - 1) * Bytespp] <= ErrodeIfSmaller)
						)
					{
						TPixels[y * pitch + x * Bytespp + 0] = 255;
						TPixels[y * pitch + x * Bytespp + 1] = 255;
...
int IsSameImageContent(BYTE* Pixels1, int Width1, int Height1, int pitch1, BYTE* Pixels2, int Width2, int Height2, int pitch2)
{
	...
	for(int y=0;y<Height1;y++)
		for (int x = 0; x < Width1; x++)
		{
			if (Pixels1[y * pitch1 + x * Bytespp + 0] != Pixels2[y * pitch1 + x * Bytespp + 0])
				return 0;
			if (Pixels1[y * pitch1 + x * Bytespp + 1] != Pixels2[y * pitch1 + x * Bytespp + 1])
				return 0;
			if (Pixels1[y * pitch1 + x * Bytespp + 2] != Pixels2[y * pitch1 + x * Bytespp + 2])
				return 0;
		}
	return 1;
}
